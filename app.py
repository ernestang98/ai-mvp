...
def gen():
    ...
    while vs.stream.isOpened():
        ...
        if img is not ():
            ...
            for facebox in faceboxes:
                ...
                if SPIT:
                    ...
                    if END_TIME_CALIBRATE is None:
                        END_TIME_CALIBRATE = datetime.datetime.now() + datetime.timedelta(
                            seconds=DURATION_TO_CALIBRATE)
            ...
            img = cv2.resize(cv2.flip(img, 1), (0, 0), fx=0.75, fy=0.75)
    ...
    UI Design purposes
    """
    # cap = cv2.VideoCapture("./static/videos/sample.avi")
    # while True:
    #     ret, img = cap.read()
    #     img = cv2.resize(cv2.flip(img, 1), (0, 0), fx=0.75, fy=0.75)
    #     frame = cv2.imencode('.jpg', img)[1].tobytes()
